...
#   sudo bash Setup-Ubuntu-DataMount.sh --default-env            # 4개 표준 LLM 환경변수 자동 설정
#   sudo bash Setup-Ubuntu-DataMount.sh --env HF_HOME=/mnt/data/hf  # 개별 환경변수 추가/덮어쓰기
#
# 전제 조건:
...
DRY_RUN='false'
ASSUME_YES='false'
APPLY_DEFAULT_ENV='true'
ENV_VARS=()
PROFILE_FILE='/etc/profile.d/wsl-data-mount.sh'
...
환경변수 (둘 다 미지정 시 환경변수 단계 스킵):
  --default-env          4개 표준 LLM 환경변수 자동 설정 (HF_HOME, HUGGINGFACE_HUB_CACHE,
                         LLAMA_CACHE, VLLM_CACHE_ROOT). 모두 \$MOUNT_POINT 하위
  --env KEY=VALUE        개별 환경변수 추가/덮어쓰기 (반복 사용 가능)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --default-env)   APPLY_DEFAULT_ENV='true'; shift ;;
        --env)
            [[ "$2" =~ ^[A-Za-z_][A-Za-z0-9_]*= ]] || { echo "[ERROR] --env 인자는 KEY=VALUE 형식이어야 합니다: '$2'" >&2; exit 1; }
            ENV_VARS+=("$2"); shift 2 ;;
        -h|--help)       usage; exit 0 ;;
...
# --- /etc/profile.d 환경변수 적용 ---
log_info '--- 환경변수 (/etc/profile.d) ---'
declare -A FINAL_ENV
ORDER=()

if [[ "$APPLY_DEFAULT_ENV" == 'true' ]]; then
    DEF_SET=(
        "HF_HOME=${MOUNT_POINT}/huggingface"
        "HUGGINGFACE_HUB_CACHE=${MOUNT_POINT}/huggingface/hub"
        "LLAMA_CACHE=${MOUNT_POINT}/llama"
        "VLLM_CACHE_ROOT=${MOUNT_POINT}/vllm"
    )
    for kv in "${DEF_SET[@]}"; do
        key="${kv%%=*}"; val="${kv#*=}"
        if [[ -z "${FINAL_ENV[$key]+x}" ]]; then ORDER+=("$key"); fi
        FINAL_ENV[$key]="$val"
    done
fi
...
if [[ "${#ORDER[@]}" -gt 0 ]]; then
    ...
    if [[ -f "$PROFILE_FILE" ]]; then
        run_cmd "cp -a $PROFILE_FILE ${PROFILE_FILE}.bak-$TS"
        log_info "백업: ${PROFILE_FILE}.bak-$TS"
    fi

    # 작성
    if [[ "$DRY_RUN" == 'true' ]]; then
        printf "  [DRY] write %s with %d export(s):\n" "$PROFILE_FILE" "${#ORDER[@]}"
        for key in "${ORDER[@]}"; do
            printf '    export %s=%s\n' "$key" "${FINAL_ENV[$key]}"
        done
    else
        {
            printf '# auto-generated by Setup-Ubuntu-DataMount.sh on %s\n' "$(date -Iseconds)"
            printf '# 매 실행 시 재생성되므로 수동 편집은 덮어써집니다.\n'
            printf '# 사용자 정의가 필요하면 별도 파일을 만드십시오 (예: /etc/profile.d/local.sh).\n'
            for key in "${ORDER[@]}"; do
                printf 'export %s=%q\n' "$key" "${FINAL_ENV[$key]}"
            done
        } > "$PROFILE_FILE"
    ...
    log_ok "$PROFILE_FILE 갱신 (${#ORDER[@]}개 export)"

    # 디렉터리 자동 생성 + chown (MOUNT_POINT 하위 값만)
    for key in "${ORDER[@]}"; do
        val="${FINAL_ENV[$key]}"
        case "$val" in
            "$MOUNT_POINT"|"$MOUNT_POINT"/*)
                run_cmd "mkdir -p $val"
                if id -u "$OWNER" >/dev/null 2>&1; then
                    run_cmd "chown -R ${OWNER}:${OWNER} $val"
                fi
                log_ok "디렉터리 준비: $val ($key)"
                ;;
            *)
                log_info "$key=$val 는 $MOUNT_POINT 외부 — 디렉터리 자동 생성 스킵"
                ;;
        esac
    done
    PROFILE_APPLIED='true'
else
    log_info '환경변수 옵션 미지정 (--default-env / --env). 단계 스킵.'
fi
...
if [[ "$PROFILE_APPLIED" == 'true' ]]; then
    echo "  환경변수       : $PROFILE_FILE (${#ORDER[@]}개)"
    for key in "${ORDER[@]}"; do
        printf '    %-25s = %s\n' "$key" "${FINAL_ENV[$key]}"
    done
fi
...
if [[ "$PROFILE_APPLIED" == 'true' ]]; then
    echo "  source $PROFILE_FILE && env | grep -E 'HF_HOME|HUGGINGFACE_HUB_CACHE|LLAMA_CACHE|VLLM_CACHE_ROOT'"
fi
